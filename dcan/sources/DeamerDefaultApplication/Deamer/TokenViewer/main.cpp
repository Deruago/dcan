...
#include "dcan/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = dcan::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		dcan::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << dcan::ast::relation::ConvertEnumToText(static_cast<dcan::ast::Type>(token->Type)) << "\n";
